...
describe("getToken()", () => {
  const ORIGINAL_TOKEN = process.env.SHORTLY_MCP_TOKEN;

  beforeEach(() => {
    delete process.env.SHORTLY_MCP_TOKEN;
  });
  ...
  afterEach(() => {
    if (ORIGINAL_TOKEN === undefined) {
      ...
      process.env.SHORTLY_MCP_TOKEN = ORIGINAL_TOKEN;
    }
  });
  ...
  it("returns the env var value when no ALS context", () => {
    process.env.SHORTLY_MCP_TOKEN = "env-jwt";
