...
An MCP (Model Context Protocol) server for the Mikku URL shortener. Two transports:
...
> **Migrating from 0.2.x?** The `register_user` and `login` tools have been removed. Get a JWT from the Mikku web app and configure your MCP client to send it (see [Getting a token](#getting-a-token)).

## Prerequisites

- Node.js 18+
- The Express backend (`../app`) running, either locally or deployed
- A JWT from the Mikku web app — see [Getting a token](#getting-a-token)
...
All tools require a JWT. The MCP server does **not** issue tokens — obtain one from the Mikku web app, then configure your client to send it.
...
```json
{
  "mcpServers": {
    "mikku": {
      "url": "https://mikku-mcp.onrender.com/mcp",
      "headers": {
...
7. After first deploy, hit `https://mikku-mcp.onrender.com/health` to confirm
